...
|----|----------------|------|-----------------|-----------|----|----|----------|
| H-37 | Engage (VeData) | Wire | Bắn sự-kiện tương tác từ bề mặt ProofChat lên `POST /v1/engage/events` (body `InteractionEvent[]` hoặc `{events:[…]}` → `202 {ok,accepted}`; event sai schema → `422` **fail-closed cả lô**, client phải validate trước khi gửi và phải có đường hiện lỗi thật, không nuốt). **ĐỪNG xếp lịch dựng** — chưa có URL công khai, đừng ghim domain nào vào client. Chặn thật KHÔNG nằm ở bước deploy mà ở tầng dưới: chưa có URL Mirage/Strata ⇒ service rơi về `FileSink`, `anchored:false` (`Engage/src/service/sinks.ts:53`), tức sự-kiện chưa có bằng chứng bất biến nào. Cửa gom **5 phút** rồi mới seal ⇒ giao diện KHÔNG được hứa "đã ghi bất biến" ngay lúc bấm. Dòng này để sổ khỏi thiếu một module; VeData sẽ gửi thư đổi 🔴→🟡/🟢 kèm số đo khi có host. | `VeDataIO/Engage/README.md` · `Engage/Specs/Engage-Spec-v1.md` | 🔴 | (chưa phân) | 2026-08-15 |
