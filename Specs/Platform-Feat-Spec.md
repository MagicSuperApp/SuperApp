...
>
> **Dependency tường minh F8.3 (hết nợ-spec-ẩn — giải FZ-11)**: F8.3 GIỮ **Must**. ProofChat hiện CHƯA có spec chính tắc (doc nhúng trong code) → **Phase 1 KÉO THEO việc team ProofChat sản xuất ProofChat spec** (Feat Chat + tích hợp SG8 theo INTEGRATION-STANDARD). Đây KHÔNG còn là scope ẩn: yêu cầu spec đã được phát tới team ProofChat (draft `ProofChat-SpecRequest.md` — kênh thư CŨ `_team-messages/`, đã ngừng, ngoài repo; nay dùng `_Agents/inbox/`). F8.3 là dependency tường minh, KHÔNG phải nợ ngầm trong lane Feat. **Cập nhật 2026-08-10: dependency ĐÃ ĐÓNG** — ProofChat đã publish contract chính tắc `ProofChat/INTEGRATION.md` bản **2026-08-08** (thay bản 2026-07-04). Đó là nguồn duy nhất; đọc mục ⚠️ đầu tài liệu trước vì bản cũ dạy sai 4 chỗ.
...
- **FZ-11** ✓ ĐÃ ĐÓNG (v0.3) — Founder chốt hướng: F8.3 GIỮ **Must** + "Phase 1 KÉO THEO team ProofChat sản xuất ProofChat spec" (đã yêu cầu, draft `ProofChat-SpecRequest.md` ở kênh thư cũ `_team-messages/`, ngoài repo). Spec đó nay ĐÃ CÓ: `ProofChat/INTEGRATION.md` bản 2026-08-08. Dependency tường minh ghi ở F8.3 + ghi chú dưới bảng FG8. Hết nợ-spec-ẩn trong lane Feat.
