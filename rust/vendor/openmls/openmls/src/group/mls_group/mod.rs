...
pub struct MlsGroup {
    ...
    /// [chat_mls patch] Raw bytes of the key-schedule `epoch_secret` (RFC 9420
    /// §8) for the current epoch. Retained in memory to prove cross-impl
    /// interop against ts-mls. `None` for groups loaded from storage (this
    /// value is transient and not persisted).
    epoch_secret: Option<Vec<u8>>,
}
...
impl MlsGroup {
    ...
    /// [chat_mls patch] Returns the raw bytes of the key-schedule
    /// `epoch_secret` (RFC 9420 §8) for the current epoch, if retained.
    ///
    /// This is deliberately exposed for cross-implementation interop testing
    /// (proving the value matches ts-mls). Returns `None` for groups loaded
    /// from storage, since the value is transient and not persisted.
    pub fn epoch_secret(&self) -> Option<&[u8]> {
        self.epoch_secret.as_deref()
    }
    ...
    pub fn load<Storage: crate::storage::StorageProvider>(
        ...
        let build = || -> Option<Self> {
            Some(Self {
                ...
                // [chat_mls patch] transient value, not persisted
                epoch_secret: None,
            })
        };
...
pub struct StagedWelcome {
    ...
    /// [chat_mls patch] Raw bytes of the key-schedule `epoch_secret` for the
    /// joined epoch, carried through to the resulting [`MlsGroup`].
    epoch_secret: Vec<u8>,
}
