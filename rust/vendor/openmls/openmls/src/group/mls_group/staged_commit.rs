...
impl MlsGroup {
    ...
    fn stage_applied_proposal_values(
        ...
        let EpochSecretsResult {
            ...
            // [chat_mls patch] capture raw epoch_secret để mang qua merge
            epoch_secret,
    ...
    pub(crate) fn merge_commit<Provider: OpenMlsProvider>(
        ...
        match staged_commit.state {
            ...
            StagedCommitState::GroupMember(state) => {
                ...
                // [chat_mls patch] cập nhật epoch_secret cho epoch mới sau merge.
                self.epoch_secret = Some(state.epoch_secret);
...
pub(crate) struct MemberStagedCommitState {
    // [chat_mls patch] Raw key-schedule epoch_secret của epoch mới, mang qua merge
    // để `MlsGroup::epoch_secret()` đúng sau khi commit (add/update/remove) được merge.
    #[serde(default)]
    epoch_secret: Vec<u8>,
