...
pub(crate) struct EpochSecretsResult {
    ...
    // §8) for the current epoch, retained before it is consumed into the
    // derived EpochSecrets. Used to prove interop against ts-mls.
    pub(crate) epoch_secret: Vec<u8>,
    #[cfg(feature = "extensions-draft-08")]
...
impl KeySchedule {
    ...
    pub(crate) fn epoch_secrets(
        ...
        // [chat_mls patch] Capture the raw epoch_secret bytes BEFORE
        // `EpochSecrets::new` consumes `epoch_secret`.
        let epoch_secret_bytes = epoch_secret.as_slice().to_vec();
        ...
        let res = EpochSecretsResult {
            ...
            // [chat_mls patch]
            epoch_secret: epoch_secret_bytes,
        };
...
impl EpochSecret {
    ...
    // [chat_mls patch] Expose the raw bytes of the epoch_secret so the group
    // can retain a copy before this EpochSecret is consumed into derived
    // secrets. Needed for cross-implementation (ts-mls) key-schedule interop.
    pub(crate) fn as_slice(&self) -> &[u8] {
        self.secret.as_slice()
    }
}
