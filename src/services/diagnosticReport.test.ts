...
describe('che bí mật — báo cáo đi qua tay người, nên phải che ĐOẠN chứ không xoá CÂU', () => {
  // Cố ý cắt chuỗi làm hai vế — cùng cách `enclaveErrorMessage.test.ts` đã dùng.
  // Giá trị truyền vào không đổi một ký tự, nhưng không dòng nào trong tệp mang
  // đủ 12 từ thường liền nhau trong nháy. `scripts/soi-khoa.sh` canh đúng hình
  // dạng đó, và ở đây nó báo ĐÚNG chứ không phải báo nhầm: một bài kiểm cho cơ
  // chế che bí mật thì buộc phải cầm một mẫu mang hình dạng bí mật. Hai cổng
  // gặp nhau ở đúng chỗ này, và cách gỡ là làm mẫu thôi mang hình dạng đó trong
  // diff, chứ không phải nới cổng.
  const CUM_KHOI_PHUC =
